...
void SPU_struct::KeyOn(int channel)
{
    ...
    TWOSF_resampler_clear(thischan.resampler);
    TWOSF_resampler_set_quality(thischan.resampler, thischan.format == 3 ? TWOSF_RESAMPLER_QUALITY_BLEP : spuInterpolationMode(state));
...
static FORCEINLINE void TestForLoop(NDS_state *state, int FORMAT, SPU_struct *SPU, channel_struct *chan)
{
	...
	if (chan->sampcnt > chan->double_totlength_shifted)
	{
		...
		{
			if (!chan->resampler || !TWOSF_resampler_get_sample_count(chan->resampler))
			{
...
static FORCEINLINE void Fetch8BitData(SPUInterpolationMode INTERPOLATE_MODE, NDS_state *state, SPU_struct* const SPU, channel_struct *chan, s32 *data)
{
	...
	TWOSF_resampler_set_rate( chan->resampler, saved_inc );

	while (chan->status != CHANSTAT_EMPTYBUFFER && TWOSF_resampler_get_free_count(chan->resampler))
	{
		s32 sample;
		Fetch8BitDataInternal(chan, &sample);
		TestForLoop(state, 0, SPU, chan);
		TWOSF_resampler_write_sample(chan->resampler, sample);
	}

	chan->sampinc = saved_inc;

	if (!TWOSF_resampler_get_sample_count(chan->resampler))
	{
		chan->status = CHANSTAT_STOPPED;
		if(SPU == state->SPU_core)
			state->MMU->ARM7_REG[0x403 + (((chan-SPU->channels) ) * 0x10)] &= 0x7F;
	...
	*data = TWOSF_resampler_get_sample(chan->resampler);
	TWOSF_resampler_remove_sample(chan->resampler, 1);
}
...
{
	TWOSF_resampler_set_rate( chan->resampler, chan->sampinc );
    
	while (TWOSF_resampler_get_free_count(chan->resampler))
	{
		s32 sample;
