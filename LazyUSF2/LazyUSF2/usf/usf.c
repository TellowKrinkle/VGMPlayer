...
void usf_clear(void * state)
{
    ...
    USF_STATE->resampler = USF_resampler_create();
...
const char * usf_render_resampled(void * state, int16_t * buffer, size_t count, int32_t sample_rate)
{
    if ( !buffer )
    {
        unsigned long samples_buffered = USF_resampler_get_sample_count( USF_STATE->resampler );
        USF_resampler_clear(USF_STATE->resampler);
        if (samples_buffered)
        {
            ...
            while (samples_to_remove--)
                USF_resampler_remove_sample(USF_STATE->resampler);
            if (!count)
                return 0;
        }
    ...
    while ( count )
    {
        ...
        while ( USF_STATE->samples_in_buffer_2 && USF_resampler_get_free_count(USF_STATE->resampler) )
        {
            int i = 0, j = USF_resampler_get_free_count(USF_STATE->resampler);
            if (j > USF_STATE->samples_in_buffer_2)
                j = (int)USF_STATE->samples_in_buffer_2;
            for (i = 0; i < j; ++i)
            {
                USF_resampler_write_sample(USF_STATE->resampler, USF_STATE->samplebuf2[i*2], USF_STATE->samplebuf2[i*2+1]);
            }
        ...
        while ( count && USF_resampler_get_sample_count(USF_STATE->resampler) )
        {
            USF_resampler_get_sample(USF_STATE->resampler, buffer, buffer + 1);
            USF_resampler_remove_sample(USF_STATE->resampler);
            buffer += 2;
            --count;
        }
        ...
        USF_resampler_set_rate(USF_STATE->resampler, (float)USF_STATE->SampleRate / (float)sample_rate);
    }
...
#endif
    USF_resampler_delete(USF_STATE->resampler);
