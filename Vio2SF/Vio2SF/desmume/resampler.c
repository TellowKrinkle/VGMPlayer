...
enum { TWOSF_RESAMPLER_SHIFT = 10 };
enum { TWOSF_RESAMPLER_SHIFT_EXTRA = 8 };
enum { TWOSF_RESAMPLER_RESOLUTION = 1 << TWOSF_RESAMPLER_SHIFT };
enum { TWOSF_RESAMPLER_RESOLUTION_EXTRA = 1 << (TWOSF_RESAMPLER_SHIFT + TWOSF_RESAMPLER_SHIFT_EXTRA) };
enum { TWOSF_SINC_WIDTH = 16 };
enum { TWOSF_SINC_SAMPLES = TWOSF_RESAMPLER_RESOLUTION * TWOSF_SINC_WIDTH };
enum { TWOSF_CUBIC_SAMPLES = TWOSF_RESAMPLER_RESOLUTION * 4 };
...
ALIGNED static float cubic_lut[TWOSF_CUBIC_SAMPLES];

static float sinc_lut[TWOSF_SINC_SAMPLES + 1];
static float window_lut[TWOSF_SINC_SAMPLES + 1];

enum { resampler_buffer_size = TWOSF_SINC_WIDTH * 4 };
...
{
    unsigned i;
    double dx = (float)(TWOSF_SINC_WIDTH) / TWOSF_SINC_SAMPLES, x = 0.0;
    for (i = 0; i < TWOSF_SINC_SAMPLES + 1; ++i, x += dx)
    {
        float y = x / TWOSF_SINC_WIDTH;
#if 0
        // Blackman
        ...
        float window = sinc(y);
#endif
        sinc_lut[i] = fabs(x) < TWOSF_SINC_WIDTH ? sinc(x) : 0.0;
        window_lut[i] = window;
    }
    dx = 1.0 / (float)(TWOSF_RESAMPLER_RESOLUTION);
    x = 0.0;
    for (i = 0; i < TWOSF_RESAMPLER_RESOLUTION; ++i, x += dx)
    {
...
typedef struct TWOSF_resampler
{
    ...
    float buffer_out[resampler_buffer_size + TWOSF_SINC_WIDTH * 2 - 1];
} resampler;
...
void * TWOSF_resampler_create(void)
{
    ...
    r->write_pos = TWOSF_SINC_WIDTH - 1;
    r->write_filled = 0;
    r->read_pos = 0;
    r->read_filled = 0;
    r->phase = 0;
    ...
    r->inv_phase = 0;
    r->inv_phase_inc = 0;
    r->quality = TWOSF_RESAMPLER_QUALITY_MAX;
    r->delay_added = -1;
    r->delay_removed = -1;
    r->last_amp = 0;
    r->accumulator = 0;
...
void TWOSF_resampler_delete(void * _r)
{
    free( _r );
}

void * TWOSF_resampler_dup(const void * _r)
{
    void * r_out = malloc( sizeof(resampler) );
    if ( !r_out ) return 0;

    TWOSF_resampler_dup_inplace(r_out, _r);

    return r_out;
}

void TWOSF_resampler_dup_inplace(void *_d, const void *_s)
{
...
void TWOSF_resampler_set_quality(void *_r, int quality)
{
    resampler * r = ( resampler * ) _r;
    if (quality < TWOSF_RESAMPLER_QUALITY_MIN)
        quality = TWOSF_RESAMPLER_QUALITY_MIN;
    else if (quality > TWOSF_RESAMPLER_QUALITY_MAX)
        quality = TWOSF_RESAMPLER_QUALITY_MAX;
    if ( r->quality != quality )
    {
        if ( quality == TWOSF_RESAMPLER_QUALITY_BLEP || r->quality == TWOSF_RESAMPLER_QUALITY_BLEP ||
             quality == TWOSF_RESAMPLER_QUALITY_BLAM || r->quality == TWOSF_RESAMPLER_QUALITY_BLAM )
        {
            r->read_pos = 0;
            r->read_filled = 0;
            r->last_amp = 0;
...
int TWOSF_resampler_get_free_count(void *_r)
{
    resampler * r = ( resampler * ) _r;
...
static int resampler_min_filled(resampler *r)
{
    ...
    {
    default:
    case TWOSF_RESAMPLER_QUALITY_ZOH:
    case TWOSF_RESAMPLER_QUALITY_BLEP:
        return 1;
            
    case TWOSF_RESAMPLER_QUALITY_LINEAR:
    case TWOSF_RESAMPLER_QUALITY_BLAM:
        return 2;
            
    case TWOSF_RESAMPLER_QUALITY_CUBIC:
        return 4;
            
    case TWOSF_RESAMPLER_QUALITY_SINC:
        return TWOSF_SINC_WIDTH * 2;
    }
}
...
int TWOSF_resampler_ready(void *_r)
{
    resampler * r = ( resampler * ) _r;
    return r->write_filled > resampler_min_filled(r);
}

void TWOSF_resampler_clear(void *_r)
{
    resampler * r = ( resampler * ) _r;
    r->write_pos = TWOSF_SINC_WIDTH - 1;
    r->write_filled = 0;
    ...
    r->delay_added = -1;
    r->delay_removed = -1;
    memset(r->buffer_in, 0, (TWOSF_SINC_WIDTH - 1) * sizeof(r->buffer_in[0]));
    memset(r->buffer_in + resampler_buffer_size, 0, (TWOSF_SINC_WIDTH - 1) * sizeof(r->buffer_in[0]));
    if (r->quality == TWOSF_RESAMPLER_QUALITY_BLEP || r->quality == TWOSF_RESAMPLER_QUALITY_BLAM)
    {
        r->inv_phase = 0;
        r->last_amp = 0;
        r->accumulator = 0;
...
void TWOSF_resampler_set_rate(void *_r, double new_factor)
{
    resampler * r = ( resampler * ) _r;
...
void TWOSF_resampler_write_sample(void *_r, short s)
{
    resampler * r = ( resampler * ) _r;
...
void TWOSF_resampler_write_sample_fixed(void *_r, int s, unsigned char depth)
{
    resampler * r = ( resampler * ) _r;
...
static int resampler_run_blep(resampler * r, float ** out_, float * out_end)
{
    ...
    if ( in_size > 0 )
    {
        ...
        const int step = RESAMPLER_BLEP_CUTOFF * TWOSF_RESAMPLER_RESOLUTION;
        const int window_step = TWOSF_RESAMPLER_RESOLUTION;
        
        do
        {
            float sample;
            
            if ( out + TWOSF_SINC_WIDTH * 2 > out_end )
                break;
            ...
            if (sample)
            {
                float kernel[TWOSF_SINC_WIDTH * 2], kernel_sum = 0.0f;
                int phase_reduced = (int)(inv_phase * TWOSF_RESAMPLER_RESOLUTION);
                int phase_adj = phase_reduced * step / TWOSF_RESAMPLER_RESOLUTION;
                int i = TWOSF_SINC_WIDTH;

                for (; i >= -TWOSF_SINC_WIDTH + 1; --i)
                {
                    int pos = i * step;
                    int window_pos = i * window_step;
                    kernel_sum += kernel[i + TWOSF_SINC_WIDTH - 1] = sinc_lut[abs(phase_adj - pos)] * window_lut[abs(phase_reduced - window_pos)];
                }
                last_amp += sample;
                sample /= kernel_sum;
                for (i = 0; i < TWOSF_SINC_WIDTH * 2; ++i)
    ...
    if ( in_size > 0 )
    {
        ...
        do
        {
            ...
            if (sample)
            {
                ...
                __m128 kernel[TWOSF_SINC_WIDTH / 2];
                __m128 temp1, temp2;
                __m128 samplex;
                ...
                for (; i >= -TWOSF_SINC_WIDTH + 1; --i)
                {
                    int pos = i * step;
                    int window_pos = i * window_step;
                    kernel_sum += kernelf[i + TWOSF_SINC_WIDTH - 1] = sinc_lut[abs(phase_adj - pos)] * window_lut[abs(phase_reduced - window_pos)];
                }
                last_amp += sample;
                sample /= kernel_sum;
...
static int resampler_run_blam(resampler * r, float ** out_, float * out_end)
{
    ...
    if ( in_size > 0 )
    {
        ...
        const int step = RESAMPLER_BLAM_CUTOFF * TWOSF_RESAMPLER_RESOLUTION;
        const int window_step = TWOSF_RESAMPLER_RESOLUTION;
...
static int resampler_run_cubic(resampler * r, float ** out_, float * out_end)
{
    ...
    if ( in_size > 0 )
    {
        ...
        do
        {
            ...
            kernel = cubic_lut + (int)(phase * TWOSF_RESAMPLER_RESOLUTION) * 4;
...
static int resampler_run_cubic_sse(resampler * r, float ** out_, float * out_end)
{
    ...
    if ( in_size > 0 )
    {
        ...
        do
        {
            ...
            temp2 = _mm_load_ps( (const float *)( cubic_lut + (int)(phase * TWOSF_RESAMPLER_RESOLUTION) * 4 ) );
...
static int resampler_run_sinc(resampler * r, float ** out_, float * out_end)
{
    ...
    if ( in_size > 0 )
    {
        ...
        int step = phase_inc > 1.0f ? (int)(TWOSF_RESAMPLER_RESOLUTION / phase_inc * RESAMPLER_SINC_CUTOFF) : (int)(TWOSF_RESAMPLER_RESOLUTION * RESAMPLER_SINC_CUTOFF);
        int window_step = TWOSF_RESAMPLER_RESOLUTION;

        do
        {
            float kernel[TWOSF_SINC_WIDTH * 2], kernel_sum = 0.0;
            int i = TWOSF_SINC_WIDTH;
            int phase_reduced = (int)(phase * TWOSF_RESAMPLER_RESOLUTION);
            int phase_adj = phase_reduced * step / TWOSF_RESAMPLER_RESOLUTION;
            float sample;
            ...
            for (sample = 0, i = 0; i < TWOSF_SINC_WIDTH * 2; ++i)
                sample += in[i] * kernel[i];
...
static void resampler_fill(resampler * r)
{
            ...
            r->read_filled < resampler_buffer_size )
    {
        ...
        case TWOSF_RESAMPLER_QUALITY_BLEP:
        {
            ...
            if ( write_extra > TWOSF_SINC_WIDTH * 2 - 1 )
                write_extra = TWOSF_SINC_WIDTH * 2 - 1;
...
static void resampler_fill_and_remove_delay(resampler * r)
{
    ...
    if ( r->delay_removed < 0 )
    {
        ...
        r->delay_removed = 0;
        while ( delay-- )
            TWOSF_resampler_remove_sample( r, 1 );
    }
}

int TWOSF_resampler_get_sample_count(void *_r)
{
    resampler * r = ( resampler * ) _r;
    if ( r->read_filled < 1 && ((r->quality != TWOSF_RESAMPLER_QUALITY_BLEP && r->quality != TWOSF_RESAMPLER_QUALITY_BLAM) || r->inv_phase_inc))
        resampler_fill_and_remove_delay( r );
    return r->read_filled;
}

int TWOSF_resampler_get_sample(void *_r)
{
    resampler * r = ( resampler * ) _r;
...
float TWOSF_resampler_get_sample_float(void *_r)
{
    resampler * r = ( resampler * ) _r;
    if ( r->read_filled < 1 && r->phase_inc)
        resampler_fill_and_remove_delay( r );
...
void TWOSF_resampler_remove_sample(void *_r, int decay)
{
    resampler * r = ( resampler * ) _r;
    if ( r->read_filled > 0 )
    {
